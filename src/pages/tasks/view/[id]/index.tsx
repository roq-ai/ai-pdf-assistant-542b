...
import { ReminderListPage } from 'pages/reminders';
...
function TaskViewPage() {
  ...
  const queryParams = useMemo<{ include: { assistant: boolean } }>(
    () =>
          ...
          <>
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              ...
              >
                ...
                <FormListItem label="Due Date" text={data?.due_date ? format(data?.due_date, 'dd-MM-yyyy') : ''} />
                ...
                {hasAccess('assistant', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="Assistant"
                    text={
                      <Link as={NextLink} href={`/assistants/view/${data?.assistant?.id}`}>
                        {data?.assistant?.name + ''}
                      </Link>
                    }
                  />
