...
import { useRoqClient, useReminderFindManyWithCount } from 'lib/roq';
import { convertQueryToPrismaUtil } from 'lib/utils';

type ColumnType = ColumnDef<ReminderInterface, unknown>;
...
export function ReminderListPage(props: ReminderListPageProps) {
  const {
    filters = {},
    titleProps = {},
      ...
      convertQueryToPrismaUtil(
        {
          ...
          searchTermKeys: ['title.contains', 'description.contains'],
          ...params.filters,
        },
        'reminder',
      ),
  ...
  const { data, error, isLoading, mutate } = useReminderFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.reminder.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const columns: ColumnType[] = [
    ...
    { id: 'description', header: 'Description', accessorKey: 'description' },
    {
      ...
      cell: ({ row: { original: record } }: any) =>
        record?.reminder_time ? format(record?.reminder_time, 'dd-MM-yyyy') : '',
    },
    hasAccess('task', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          accessorKey: 'task',
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/tasks/view/${record.task?.id}`}>
              {record.task?.title}
            </Link>
          ),
        }
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/reminders/view/${record.id}`} passHref legacyBehavior>
                <Button
