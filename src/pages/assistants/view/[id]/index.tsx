...
import { useAssistantFindFirst, useRoqClient } from 'lib/roq';

import { AssistantInterface } from 'interfaces/assistant';
import { TaskListPage } from 'pages/tasks';

function AssistantViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const roqClient = useRoqClient();
  const queryParams = useMemo<{ include: { pdf: boolean; user: boolean } }>(
    () =>
  ...
  const { data, error, isLoading } = useAssistantFindFirst(queryParams, {}, { disabled: !id });
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('assistant', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/assistants/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              >
                ...
                {hasAccess('pdf', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="Pdf"
                    text={
                      <Link as={NextLink} href={`/pdfs/view/${data?.pdf?.id}`}>
                        {data?.pdf?.title + ''}
                      </Link>
                    }
                  />
