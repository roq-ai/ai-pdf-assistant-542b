...
import { useRoqClient, useReminderFindFirst } from 'lib/roq';
...
function ReminderEditPage() {
  ...
  const { data, error, isLoading, mutate } = useReminderFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: ReminderInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.reminder.update({
        data: values as RoqTypes.reminder,
        where: {
          id,
        },
      });
