...
export function TaskListPage(props: TaskListPageProps) {
  ...
  const queryParams = useMemo(
    () =>
      convertQueryToPrismaUtil(
        {
          relations: ['assistant', 'reminder.count'],
          searchTerm: params.searchTerm,
          searchTermKeys: ['title.contains', 'description.contains', 'status.contains'],
          ...params.filters,
        },
        'task',
      ),
  ...
  const columns: ColumnType[] = [
    ...
    { id: 'status', header: 'Status', accessorKey: 'status' },
    {
      ...
      cell: ({ row: { original: record } }: any) => (record?.due_date ? format(record?.due_date, 'dd-MM-yyyy') : ''),
    },
    hasAccess('assistant', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/assistants/view/${record.assistant?.id}`}>
    ...
    hasAccess('reminder', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          cell: ({ row: { original: record } }: any) => record?._count?.reminder,
        }
      : null,
