...
import { useRoqPlatformUserProfiles, useTaskCount, useAssistantCount, useReminderCount } from 'lib/roq';

import { AssistantListPage as ShowCaseList } from 'pages/assistants';
...
function DashboardPage() {
  ...
  const assistantStat = useAssistantCount();

  const reminderStat = useReminderCount();
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Assistants',
      result: assistantStat?.data,
      isLoading: assistantStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Reminders',
      result: reminderStat?.data,
      isLoading: reminderStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`assistants`} passHref legacyBehavior>
                  <Button
