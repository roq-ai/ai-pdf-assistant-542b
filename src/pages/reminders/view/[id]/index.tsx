...
import { useReminderFindFirst, useRoqClient } from 'lib/roq';

import { ReminderInterface } from 'interfaces/reminder';

function ReminderViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const roqClient = useRoqClient();
  const queryParams = useMemo<{ include: { task: boolean } }>(
    () =>
  ...
  const { data, error, isLoading } = useReminderFindFirst(queryParams, {}, { disabled: !id });
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('reminder', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/reminders/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              >
                ...
                <FormListItem
                  label="Reminder Time"
                  text={data?.reminder_time ? format(data?.reminder_time, 'dd-MM-yyyy') : ''}
                />
                ...
                {hasAccess('task', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="Task"
                    text={
                      <Link as={NextLink} href={`/tasks/view/${data?.task?.id}`}>
                        {data?.task?.title + ''}
                      </Link>
                    }
                  />
