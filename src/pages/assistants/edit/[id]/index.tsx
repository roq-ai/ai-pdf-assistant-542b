...
import { useRoqClient, useAssistantFindFirst } from 'lib/roq';
...
function AssistantEditPage() {
  ...
  const { data, error, isLoading, mutate } = useAssistantFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: AssistantInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.assistant.update({
        data: values as RoqTypes.assistant,
        where: {
          id,
        },
      });
      ...
      <Box rounded="md">
        ...
        <FormWrapper onSubmit={formik.handleSubmit}>
          ...
          <AsyncSelect<PdfInterface>
            ...
            fetcher={() => roqClient.pdf.findManyWithCount({})}
            labelField={'title'}
          />
