...
import { useRoqClient, usePdfFindManyWithCount } from 'lib/roq';
import { convertQueryToPrismaUtil } from 'lib/utils';

type ColumnType = ColumnDef<PdfInterface, unknown>;
...
export function PdfListPage(props: PdfListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const queryParams = useMemo(
    () =>
      convertQueryToPrismaUtil(
        {
          relations: ['user', 'business', 'assistant.count'],
          searchTerm: params.searchTerm,
          searchTermKeys: ['title.contains', 'content.contains'],
          ...params.filters,
        },
        'pdf',
      ),
  ...
  const { data, error, isLoading, mutate } = usePdfFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.pdf.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const handleView = (row: PdfInterface) => {
    if (hasAccess('pdf', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/pdfs/view/${row.id}`);
    }
  };
  ...
  const columns: ColumnType[] = [
    ...
    { id: 'content', header: 'Content', accessorKey: 'content' },
    {
      ...
      cell: ({ row: { original: record } }: any) =>
        record?.upload_time ? format(record?.upload_time, 'dd-MM-yyyy') : '',
    },
    ...
    hasAccess('assistant', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          cell: ({ row: { original: record } }: any) => record?._count?.assistant,
        }
      : null,
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/pdfs/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('pdf', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/pdfs/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('pdf', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="pdf" />
        </Flex>

        {hasAccess('pdf', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/pdfs/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
