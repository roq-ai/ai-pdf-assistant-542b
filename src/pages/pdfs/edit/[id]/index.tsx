...
import { useRoqClient, usePdfFindFirst } from 'lib/roq';
...
function PdfEditPage() {
  ...
  const { data, error, isLoading, mutate } = usePdfFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: PdfInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.pdf.update({
        data: values as RoqTypes.pdf,
        where: {
          id,
        },
      });
